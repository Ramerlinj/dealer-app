      ...
      {loading ? (
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 4 }, (_, index) => (
            ...
            >
              <div className="mb-6 h-56 w-full rounded-2xl bg-slate-100" />
        ...
        <div className="rounded-3xl border border-slate-100 bg-white/70 px-6 py-10 text-center text-sm text-slate-600 shadow-[0_20px_45px_-35px_rgba(15,46,87,0.45)]">
          <p>
            No encontramos dealers para la provincia seleccionada. Ajusta el
            filtro para ver más resultados.
          </p>
        </div>
      ) : (
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {filteredDealers.map((dealer, index) => (
            <DealerCard key={dealer.id} dealer={dealer} delay={index * 0.08} />
          ))}
        </div>
      )}
    </motion.div>
